...
public class ExcursionController {
    ...
    @ResponseStatus(HttpStatus.CREATED)  // ← ДОБАВЛЕНО: возвращает 201
    ...
    public ExcursionResponse update(@PathVariable Integer id,
                                    @RequestBody @Valid ExcursionRequest request) {
        return tourServiceService.updateExcursion(id, request);
    }

    @Operation(summary = "Удаляет экскурсию по ID")
    @DeleteMapping(path = "/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)  // ← ДОБАВЛЕНО: возвращает 204
